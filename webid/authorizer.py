...
class DirectTrust(object):
    ...
    def is_trusted(self):
        ...
        """
        warnings.warn("The method has been deprecated, use Trust class or __trust callback",
    ...
    def is_trusted(self):
        ...
        for auth_owner in self.authorizer_owners:
            ...
            if not self.check_for_link(auth_profile, self.own_uri):
                logger.debug("Authorizer claims that %s is its owner but is NOT, skipping..."
                             % (auth_owner,))
                continue
            ...
            for common in common_friends:
                ...
                if self.check_for_link(common_profile, self.san_uri):
                    ...
                    logger.info("Supplicant claims that %s is its owner but it turned out NOT " %
                                (common,))
    ...
    def same_owner(self):
        ...
        for common in common_owners:
            ...
            if self.check_for_link(webid_from_cache(common, fresh=True, verify_server_cert=False), self.supp_uri, True):
                logger.info("SAME_OWNER: Auth:%s  and Supp:%s have same owner:%s",
    ...
    if t.is_trusted:
        ...
        if len(t.mac_list) == 1:
            try:
                ...
                d.uri = supp_uri
                # TODO: remove all the existing owners and add these new ones. Since the
                # owners of the device might have been changed.
